(function () {
  ...
  const ANY_PLACEHOLDER_TOKEN_REGEX = /\[(?:PWM|[A-Z][A-Z0-9_]*)_\d+\]/g;
  ...
  function canonicalizePlaceholderToken(token) {
    ...
    if (isPwmPlaceholder(normalized)) {
      return normalized;
    }
  ...
  function containsLegacyTypedPlaceholder(text) {
    return LEGACY_TYPED_PLACEHOLDER_REGEX.test(String(text || ""));
  }
  ...
  class PlaceholderManager {
    ...
    trackKnownPlaceholder(placeholder) {
      ...
      if (!isPwmPlaceholder(canonical)) return canonical;
    ...
    setPublicState(state = {}) {
      ...
      this.counters = { PWM: Number(state?.counters?.PWM || 0) };
    ...
    getPlaceholder(rawValue) {
      ...
      const next = Number(this.counters.PWM || 0) + 1;
      this.counters.PWM = next;

      const placeholder = `[PWM_${next}]`;
    ...
    segmentText(text) {
      ...
      let lastIndex = 0;
      let match;
      const regex = new RegExp(PWM_PLACEHOLDER_REGEX.source, "g");
  ...
  root.PWM.PLACEHOLDER_TOKEN_REGEX = PWM_PLACEHOLDER_REGEX;
  ...
  if (typeof module !== "undefined" && module.exports) {
    ...
    module.exports.PLACEHOLDER_TOKEN_REGEX = PWM_PLACEHOLDER_REGEX;

...
- WhatsApp Web supports protected text typing, multiline text, text paste, clipboard image paste for PNG/JPG/JPEG/WEBP, attach-button single file, attach-button multi-file batches, drag/drop single file, and drag/drop multi-file batches using the same small/large batch caps as other protected adapter paths; over-cap batches and unsupported file families block before read with no raw fallback
...
- WhatsApp attach/drop paths support single-file and multi-file sanitized handoff within the same protected-site small/large batch caps; over-cap batches block before read, and WhatsApp file paste remains out of scope except clipboard image paste

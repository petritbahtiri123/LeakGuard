...
WhatsApp Web has its own narrower support matrix: typing, multiline text, text paste, clipboard image paste, attach-button single file, attach-button 2-5 files, drag/drop single file, and drag/drop 2-5 files are supported; 6+ files and unsupported file families block before read; file paste is out of scope except clipboard image paste. See [WHATSAPP_SUPPORT_MATRIX.md](WHATSAPP_SUPPORT_MATRIX.md).

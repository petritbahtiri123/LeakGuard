...
WhatsApp Web has its own narrower support matrix by entry path: typing, multiline text, text paste, clipboard image paste, attach-button single file, attach-button multi-file batches, drag/drop single file, and drag/drop multi-file batches are supported within the protected-site small/large batch caps; over-cap batches and unsupported file families block before read; file paste is out of scope except clipboard image paste. See [WHATSAPP_SUPPORT_MATRIX.md](WHATSAPP_SUPPORT_MATRIX.md).

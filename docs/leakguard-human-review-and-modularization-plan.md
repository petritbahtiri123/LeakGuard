...
| File handling and unsupported-format messaging | The release explicitly supports scoped text, document-text, and PNG/JPG/JPEG/WEBP image OCR paths; unsupported-file UX must not imply protection. | `src/shared/fileScanner.js`, `src/shared/streamingFileRedactor.js`, `src/content/file_paste_helpers.js`, `src/content/content.js`, `docs/FILE_UPLOAD_SCANNING_GUIDE.md`, `docs/file-handoff-architecture.md` | High | Manually test supported UTF-8 text, invalid UTF-8, over-limit files, text PDF, DOCX, XLSX, image OCR, archives, executables, unsupported images, and binary files. Confirm raw upload blocks only in the promised fail-closed paths. |
...
| --- | --- | --- | --- | --- |
| Removed stale Gemini editor-drop path | `maybeHandleGeminiEditorDrop()` plus `listGeminiDropFiles()`, `isSupportedGeminiTextFile()`, and `readGeminiTextFile()` were removed after proving production drop listeners route through `maybeHandleDrop()`. | No remaining removal work for this candidate. | `node tests/content_file_drop_interception.test.js`; QA guard verifies drop routing. | Done. |

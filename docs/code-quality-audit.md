...
| CQ-05 | medium | dead-code | `src/content/content.js`, `tests/content_file_drop_interception.test.js` | `maybeHandleGeminiEditorDrop()` is declared at `6214`; helper functions `listGeminiDropFiles()`, `isSupportedGeminiTextFile()`, and `readGeminiTextFile()` are nearby (`6162-6213`). `rg` found no production call to `maybeHandleGeminiEditorDrop`; tests mostly extract these functions into harnesses (`tests/content_file_drop_interception.test.js` around `1034-1038`). | This looks like a stale Gemini text-drop path beside the newer generic file handoff and pending attach flow. Removing it without proof is risky because Gemini DOM behavior changes frequently, but keeping stale fallback code makes future audits harder. | Mark as candidate dead code only. Prove with a call graph, harness coverage review, and manual Gemini drop smoke before deletion. If removed, do it in a dedicated PR with no behavior changes elsewhere. | medium |
...
5. Mark `maybeHandleGeminiEditorDrop()` and its helpers as candidate dead code in an internal comment or follow-up issue after call-graph proof.

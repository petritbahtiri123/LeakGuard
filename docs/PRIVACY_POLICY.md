...
LeakGuard may also access a local file only after you choose it in the File Scanner page or paste, drop, or select it through a protected AI composer. This release supports text/source files, text PDF extraction, DOCX text extraction, XLSX text extraction, image metadata scanning, English-only scanner image OCR, and opt-in/default-off protected-site image OCR. The File Scanner can export a redacted text copy, a sanitized JSON findings report, or an eligible flattened `.redacted.png` scanner visual redaction output.

LeakGuard scans and redacts supported files locally. Scanner text PDFs can export `.redacted.txt` plus regenerated `.redacted.pdf` output built from sanitized extracted text only. Protected-site text PDFs can hand off regenerated `.redacted.pdf` output only when the regenerated PDF is complete; otherwise LeakGuard falls back to `.redacted.txt` or blocks raw upload. DOCX, XLSX, image metadata, and OCR text export `.redacted.txt`, not rebuilt original documents. Protected-site image redaction uploads `.redacted.png` only when the user has enabled protected-site OCR and OCR boxes are eligible. LeakGuard does not provide scanned-PDF OCR, non-English OCR, remote OCR, backend file processing, layout-preserving PDF redaction, DOCX/XLSX rebuilds, or image format preservation.
...
PDF, DOCX, and XLSX redaction currently means local text extraction followed by sanitized export or handoff. Scanner and protected-site text PDFs can export regenerated `.redacted.pdf` from sanitized text only; protected-site PDFs fall back to `.redacted.txt` when regeneration would truncate. LeakGuard does not rebuild DOCX or XLSX original formats. Scanner visual image redaction and eligible protected-site image redaction export flattened PNG only. Unsupported files are not marked as scanned, protected, or sanitized. Supported files above the current local limits, unreadable documents, OCR failures, and ineligible protected-site visual redaction failures are blocked rather than uploaded raw through protected file paths.

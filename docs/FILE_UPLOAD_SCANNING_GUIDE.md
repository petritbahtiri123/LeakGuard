...
- WhatsApp Web supports text typing, multiline text, text paste, clipboard image paste for PNG/JPG/JPEG/WEBP, attach-button single file, attach-button multi-file batches, drag/drop single file, and drag/drop multi-file batches within the same protected-site small/large batch caps as other adapters. WhatsApp blocks over-cap batches before read, blocks unsupported or failing batches all-or-nothing, never inserts extracted file text into the WhatsApp composer, and keeps file paste out of scope except clipboard image paste.

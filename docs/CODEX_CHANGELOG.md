...
- Notes: Added direct coverage for the Firefox/Gemini ghost-ingress, bridge input-miss, terminal handoff-failure, and hook-first no-input drop paths. Existing exposed `Filedata` inputs can still receive sanitized assignment directly; otherwise Firefox/Gemini uses the `Attach sanitized file` prompt instead of automatic upload UI clicks. No placeholder, redaction, IP, permission, manifest, or Firefox package metadata changes. Generic tools-only buttons remain rejected.

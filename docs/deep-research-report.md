...
The current repo supports the v1.3.0 hardening goals in the critical deterministic areas: trust-aware placeholder handling, sensitive HTTP header ranges, URL credential redaction, repeated raw secret placeholder reuse, and targeted natural-language/labelled disclosures. v1.4.0 release prep adds release-facing coverage for local text-file paste/drop redaction in protected AI composers: supported UTF-8 text files are validated locally, redacted through the background-owned placeholder flow, and inserted as redacted text or safely blocked on unsupported files or failed insertion. The optional AI assist is implemented as a local ONNX/classifier layer over leftover suspicious candidates, not as full-prompt cloud AI and not as a replacement for deterministic detection.
...
| Local file paste/drop redaction | `src/content/file_paste_helpers.js`, `src/content/content.js`, `src/content/composer_helpers.js`, `src/shared/fileScanner.js`, `src/background/core.js` |

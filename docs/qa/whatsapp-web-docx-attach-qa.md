...
Use this checklist for WhatsApp Web DOCX document support. The single-file path supports one DOCX selected from the attach button and handed to WhatsApp only as a rebuilt sanitized `.redacted.docx` file. The multi-file path supports sanitized files for supported DOCX files, XLSX files, PDFs, text documents, and images within the protected-site small/large batch caps.

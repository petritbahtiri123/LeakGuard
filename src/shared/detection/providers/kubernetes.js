(function () {
  ...
  function scan(text, detector) { const findings = []; const input = String(text || "");
    for (let match, re = /\b(?:pod|deployment|service|secret|configmap|ingress)\/[a-z0-9][a-z0-9.-]{1,62}\b/gi; (match = re.exec(input)) !== null;) {
      const lineStart = input.lastIndexOf("\n", Math.max(0, match.index - 1)) + 1; const lineEndIndex = input.indexOf("\n", match.index); const line = input.slice(lineStart, lineEndIndex >= 0 ? lineEndIndex : input.length); if (!hasProviderContext(input, match.index, match.index + match[0].length) && !/\b(?:pod|deployment|service|secret|configmap|ingress)\//i.test(line.replace(match[0], ""))) continue;
